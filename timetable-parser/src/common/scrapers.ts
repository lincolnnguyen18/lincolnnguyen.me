import { getCellsInTable, logInBrowser, setAttributeOfElement, setAttributeOfElements, textIsPresent, wait } from 'common/scraperUtils';
import { ChannelsUrls } from 'data/data';
import { remote } from 'webdriverio';

async function scrapeTvTokyoTimetable (closeBrowerOnFinish = true): Promise<string[][]> {
...
async function scrapeNhkgTimetable (): Promise<string[][]> {
  const browser = await remote({
    capabilities: {
      browserName: 'chrome',
    },
...
export { scrapeTvTokyoTimetable, scrapeNhkgTimetable };
