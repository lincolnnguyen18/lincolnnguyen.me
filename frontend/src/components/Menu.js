...
export function Menu () {
  ...
  const { menuOpen, menuChildren, menuChildrenHidden, menuEasyClose, menuCloseDisabled } = useSelector(commonSelector);
  ...
  return (
    <>
      ...
      <MenuContainer open={menuOpen} onClick={menuEasyClose ? handleClose : undefined} className='items-end'>
        <MenuNavbar>
          <Button onClick={handleClose} className='icon-close text-white h-11 mx-3' disabled={menuCloseDisabled} />
        </MenuNavbar>
        <div
          className='px-3 transition-[opacity] duration-100 w-full'
