  ...
  return (
    <>
      ...
      <div
        ...
        id="nav-menu"
      >
        <div className={twMerge('max-w-screen-sm mx-auto flex flex-col', navMenu.containerClassName)}>
          <Button onClick={() => dispatch(commonActions.closeNavMenu())} className={twMerge('icon-close text-white h-11 mx-3', navMenu.hideCloseButton && 'opacity-0 pointer-events-none')} disabled={navMenuCloseButtonDisabled} />
          <div className={twMerge('flex flex-col gap-3 px-3 transition-[opacity] duration-200', navMenu.menuClassName)} style={{ opacity: navMenu.hideOnlyChildren ? 0 : 1, pointerEvents: navMenu.hideOnlyChildren || !navMenu.open ? 'none' : 'all' }}>
