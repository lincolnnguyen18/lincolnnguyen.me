...
export function HomeScreen() {
  ...
  for (let i = 0; i < 50; i++) {
    dummyIcons.push(
      <AppIcon text="Resume" bgColor={theme.colors.green.custom} placeholderInitials="R" path="/resume" key={i} />,
    );
  }
  ...
  return (
    <>
      ...
      <div className="max-w-screen-sm w-full mx-auto grid grid-cols-4 grid-flow-row gap-4 place-items-center w-full px-2 pt-12 pb-4">
        <AppIcon text="Messages" bgColor={theme.colors.red.custom}  placeholderInitials="M" path="/messages/contacts" />
        <AppIcon text="Transcribe" bgColor={theme.colors.purple.custom}  placeholderInitials="T" path="/transcribe/transcriptions" />
        {dummyIcons}
      </div>
